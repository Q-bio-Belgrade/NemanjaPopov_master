...
x_pred = @(x)predicted_x(tExp, x) - noise;
optimalParams = lsqnonlin(x_pred, x0, lb, ub);
matrix(i,:) = optimalParams;
H0 = optimalParams(1);
delta_H = optimalParams(2);
n = optimalParams(3);
phi = 10;
m = 2/3 * n;
k_star = 0.02;
...
[T_eq, Z_eq] = ode23s(@(t, z_eq) CRISPR_eq(t, z_eq, m, phi, k_star, lambda_pre, k , lambda , H0 , delta_H , n, d, alpha, gamma, psi, K), tspan_eq, z0);
...
cr_sim = Z_sim(:, 8);
p1 = plot(T_sim,cr_sim,'b-',LineWidth=2);
end
...
xlabel(ax(3,3),'n');
title('Matrica distribucija parametara');
...
%%
function z_eq = CRISPR_eq( ~ , z_eq, m, phi, k_star, lambda_pre, k , lambda , H0 , delta_H , n, d, alpha, gamma, psi, K )
    
    H_hat = H0 ;

    z_eq = [
            alpha*((1+2*H_hat^m*(z_eq(4)/K))/(1+H_hat^n+H_hat^m*(z_eq(4)/K)+z_eq(2)/(4*K*50))) - lambda*z_eq(1) ; 
            k*z_eq(1) - d*z_eq(2) ; 
            gamma*((1+z_eq(2)/(4*K))/(1+H_hat^n+z_eq(2)/(4*K))) - lambda*z_eq(3) ; 
            k*z_eq(3) - d*z_eq(4) ; 
            psi*((1+z_eq(2)/(4*K))/(1+H_hat^n+z_eq(2)/(4*K))) - lambda*z_eq(5) ; 
            k*z_eq(5) - d*z_eq(6); 
